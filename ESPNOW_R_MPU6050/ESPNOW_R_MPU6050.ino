...
void loop() {
  ...
  if(SerialBT.available()){
    ...
    if (command == 1) { //drive forward
      ...
      isDriving = false;
    } else if (command == 0) { //stop or brake
      Serial.println("stop");
      isDriving = false;
    } else if (command == 4) { //print information. When car is stationary, GyroX should approx. = 0. 
      Serial.print("angle: ");
    ...
    } else if (command == 5) { //pause the program
      paused = !paused;
      stopCar();
      isDriving = false;
...
void driving (){//called by void loop(), which isDriving = true
  ...
  if (deltaAngle != 0){
    ...
    ledcWrite(0, rightSpeedVal);
    ledcWrite(1, 0);
    ledcWrite(2, leftSpeedVal);
    ledcWrite(3, 0);
  }
}
